...
![todolist](/images/todolist/todolist-3.png)
...
Typia is a library that allows type checking in real-time, even after build. The main purpose of introducing this library is to consolidate validators for DTO validation into a single object instead of separating them for testing and production. The traditional approach to testing is to determine the expected outcome in advance and write code accordingly, which reduces the reliability and value of the test. On the other hand, we decided that a more practical way to test would be to test against the same object that is being validated with real user data, and to thoroughly test various edge cases of this validated object.
