...
## 스토리북과 프론트엔드의 UI components 공유 문제 해결을 위한 npm 패키지 분리
...
Typia는 빌드 이후에도 실시간으로 타입을 체크할 수 있는 라이브러리입니다. 이 라이브러리를 도입한 주요 목적은 DTO 검증용 validator를 테스트용과 프로덕션용으로 분리하지 않고 단일 객체로 통합 관리하기 위함입니다. 기존 테스트 방식은 미리 예상 결과를 정해놓고 그에 맞게 코드를 작성하는 형태였습니다. 이러한 접근법은 테스트의 신뢰성과 가치를 저하시킵니다. 반면, 실제 사용자 데이터로 검증을 수행하는 동일한 객체를 테스트 대상으로 삼고, 이 검증 객체의 다양한 엣지 케이스를 철저히 테스트하는 것이 더 실질적인 테스트 방법이라고 판단했습니다.
